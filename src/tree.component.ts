...
export class TreeInternalComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    if(this.tree)
    {
      ...
      {
        let properties: Array<TreeModel> = this.tree.properties;
        let newNode: TreeModel = {value: 'Свойства', children: properties, _foldingType: FoldingType.PropertyExpanded, parentNode: h };
  ...
  public isPropertyFolder(): boolean {
  ...
  private isNodeExpanded(): boolean {
     // custom
    if(this.tree._foldingType === FoldingType.PropertyExpanded)
    {
      return true;
    }
    //
  ...
  private getNextFoldingType(node: TreeModel): FoldingType {
    // custom
    if (node._foldingType === FoldingType.PropertyExpanded) {
      return FoldingType.PropertyCollapsed;
    }
    else if(node._foldingType === FoldingType.PropertyCollapsed)
    {
      return FoldingType.PropertyExpanded;
    }
    //
  ...
  private onMenuItemSelected(e: NodeMenuItemSelectedEvent): void {
    switch (e.nodeMenuItemAction) {
      ...
      case NodeMenuItemAction.Remove:
        ...
        break;
      case NodeMenuItemAction.ShowObjects:
        this.onShowObjects();
        break;
      default:
...
export class TreeComponent implements OnInit {
  ...
  @Output()
  public showObjects: EventEmitter<any> = new EventEmitter();
  ...
  public ngOnInit(): void {
    ...
    this.treeService.showObjects$.subscribe((e: NodeEvent) => {

      this.showObjects.emit(e);
    });
  }
}
